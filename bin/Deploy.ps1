...
+==================================================================+
|                                                                  |
|       SETUP APPLICATION FOR [$NewBuildStage] BUILD STAGE         |
|                                                                  |
+==================================================================+
...
function Stage-Deploy
{
    ...
    } else
    {
        ...
        } else
        {
            ...
            Write-Host($BuildSetup)
            docker compose --file "$YamlPath" --env-file "$EnvPath" up --build -d

            if ($LASTEXITCODE -eq 1)
            {
                Write-Host("`n`rFAILED TO SETUP APPLICATION FOR [$BuildPath] BUILD STAGE!!")
                Exit 1
            } else
            {
                Write-Host("`n`rSUCCEED TO SETUP APPLICATION FOR [$BuildPath] BUILD STAGE!!")
            }
        }
    }
}
...
} else
{
    switch ($NewBuildStage)
    {
        'dev'
        {
            Stage-Deploy -BuildPath $NewBuildStage -YamlPath $YamlFile -EnvPath $EnvFile
        }
